	...
	private void loadAmenitiesByApartments(String contextPath) {
		...
		try {
			...
			while ((line = in.readLine()) != null) {
				...
				if (amenitiesByApartments.containsKey(apartmentId)) {
					amenitiesByApartments.get(apartmentId).add(amenity);
				} else {
					List<Amenity> apartmentAmenities = new ArrayList<Amenity>();
					apartmentAmenities.add(amenity);
					amenitiesByApartments.put(apartmentId, apartmentAmenities);
				}
			}
		} catch (Exception ex) {
			ex.printStackTrace();
